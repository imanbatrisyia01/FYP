            ...
            <main>
                <%
                    try {
                        ...
                        String studioName = ""; // Initialize the studioName variable
                        Studio bookedStudio = null;
                        // Find the studio details if a valid studio ID is provided
                        // Retrieve the selected studio's ID
                        if (studioID != null && !studioID.isEmpty()) {
                            // Fetch information about the selected studio using the studioID
                            // You can perform a database query to retrieve the studio's details based on the studioID

                            // Example database query (adjust to your database schema):
                            sql = "SELECT * FROM studioType WHERE studioID = " + studioID;

                            // Execute the query, retrieve data, and display it
                            rs = st.executeQuery(sql);
                            if (rs.next()) {
                                studioName = rs.getString("studioName");
                                bookedStudio = new Studio();
                                bookedStudio.setStudioID(rs.getInt("studioID"));
                                bookedStudio.setStudioName(studioName);
                            } else {
                                // Studio not found with the specified ID
                                out.println("Studio not found.");
                            }
                        } else {
                            // Studio ID is not provided
                            out.println("Please provide a valid studio ID.");
                        }
                        session.setAttribute("bookedStudio", bookedStudio);
                %>
                ...
                <div class="row g-5">
                    <div class="col-md-3  order-md-first lower-section" style="margin-top: 100px;">
                        ...
                        <ul class="list-group mb-3">
                            <li class="list-group-item d-flex justify-content-between lh-sm">
                                <div>
                                    <h5 class="my-0"><%= rs.getString("studioName")%></h5>
                                    <p class="mb-0"><%= rs.getString("studioDescription")%></p>
                                    <small class="text-muted">Size: <%= rs.getString("studioSize")%> sq ft</small><br>
                                    <small class="text-muted">Capacity: <%= rs.getString("capacity")%> person</small>
                                </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="row g-3">
                            <form class="needs-validation" action="checkoutStudio-process.jsp" method="post">
                                <div class="col-sm-6">
                                ...
                                <input type="hidden" id="studioID" name="studioID" value="<%= studioID%>">
                                ...
                                <tbody>
                                    <%
                                        try {
                                            ...
                                            String query = "SELECT startDateTime, finishDateTime, organization FROM studiobooking where studioID = " + studioID;
